...
const apiRequest = async (endpoint, options = {}) => {
  ...
  console.log(
    `Making API request to: ${endpoint} with cookies for authentication`
  );
  ...
  login: async (credentials) => {
    try {
      ...
      if (!response.ok) {
        console.error(
          "Login API error:",
          response.message || response.error || response.error
        );
      ...
      // Check if document and cookies are available (client-side)
